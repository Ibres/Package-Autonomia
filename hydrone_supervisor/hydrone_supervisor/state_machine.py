...
class State(Enum):
    """
    Representa os estados possíveis do veículo Hydrone conforme especificação.
    """
    # Estados Normais
    IDLE = auto()
    AERIAL_NAV = auto()
...
class StateMachine:
    ...
    Segue o princípio de que toda a lógica de transição deve existir aqui.
    """
    def __init__(self, initial_state=State.IDLE):
        self._current_state = initial_state
        self._previous_state = None
        
        # Tabela formal de transições conforme especificação
        self._transitions = {
            State.IDLE: {
            ...
            State.EMERGENCY_HOLD: {
                TransitionTrigger.SYSTEM_RECOVERED: None, # Retorna ao estado anterior
    ...
    def process_trigger(self, trigger: TransitionTrigger) -> bool:
        ...
        Retorna True se houve transição, False caso contrário.
        """
        if self._current_state in self._transitions:
            ...
            if trigger in possible_transitions:
                ...
                # Caso especial para EMERGENCY_HOLD retornar ao estado anterior
        ...
        # Verificação global para EMERGENCY_STOP (pode ser disparado de qualquer lugar por CRITICAL_FAILURE)
